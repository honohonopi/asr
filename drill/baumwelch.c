...
)
{
  ...
  for (;;)
  {
    /*------------------------------------
      forward確率αを計算
    ------------------------------------*/
    prob_forward = forward(N, M, T, O, a, b, pi, alpha);
    prob = prob_forward;
    ...
    /*------------------------------------
      収束したら再推定を終了
    ------------------------------------*/
    if (log(prob) - log(prob_old) < EPSILON)
      goto END;
    ...
    /*------------------------------------
      backward確率βを計算
    ------------------------------------*/
    prob_backward = backward(N, M, T, O, a, b, pi, beta);

    /*------------------------------------
      πi を再推定
    ------------------------------------*/
    for (i = 0; i < N; i++)
      pi[i] = pi[i] = (alpha[0][i] * beta[0][i]) / prob;
    ...
    for (i = 0; i < N; i++)
    {
      for (j = 0; j < N; j++)
      {
        ...
        for (t = 0; t < T - 1; t++)
        {
          double xi = alpha[t][i] * a[i][j] * b[j][O[t + 1]] * beta[t + 1][j];
          sum1 += xi;
...
END:
  fprintf(stderr, "Baum-Welch estimation converged. (n_reest= %d)\n", n_reest);
